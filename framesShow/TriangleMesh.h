 ...
 *  IMesh interface
 */
	virtual VertexID AppendVertex( const Vec3f & vVertex, const Vec3f * pNormal = NULL )
		{ return AppendVertexData( &vVertex, pNormal, false, NULL, &vVertex ); }
	virtual void GetVertex( VertexID vID, Vec3f & vVertex, Vec3f * pNormal = NULL ) const;
	virtual void GetNormal( VertexID vID, Vec3f & vNormal ) const;	
	...
	virtual void GetTriangle( TriangleID tID, Vec3f vTriangle[3], Vec3f * pNormals = NULL  ) const;
	...
	unsigned int
	AppendVertexData( const Vec3f * pVertex, const Vec3f * pNormal, bool bFlipNormal, 
					  const Vec2f * pTextureCoord = NULL, const Vec3f * pColor = NULL );
...
//	void GetVertex( unsigned int nVertex, Vec3f & v ) const;
//	void GetNormal( unsigned int nVertex, Vec3f & n ) const;
	void GetTextureCoords( unsigned int nVertex, Vec2f & vt ) const;
	//void GetTriangle( unsigned int nTriangle, unsigned int triangle[3] ) const;
//	void GetTriangle( unsigned int nTriangle, Vec3f * vVertices ) const;
	void GetTextureCoords( unsigned int nTriangle, Vec2f * vCoords ) const;
	void GetTriangleNormals( unsigned int nTriangle, Vec3f * vNormals );
	void GetTriTexCoords( unsigned int nTriangle, Vec2f & uv1, Vec2f & uv2, Vec2f & uv3 ) const;

	void SetTriangle( unsigned int nTriangle, unsigned int triangle[3] );
	void SetVertex( unsigned int nVertex, const Vec3f & vVertex );
	void SetNormal( unsigned int nVertex, const Vec3f & vNormal );
